...
import dev.thewarrior.Utils.Data.Spawn;
...
/**
 * Thanks to <a href="https://github.dev/nhulston/Essentials">nhulston</a> for the original TeleportUtil code!
 */
public class TeleportUtil {
     ...
     */
    @Nullable
    public static Message teleportSafe(
        ...
        if (targetWorld == null) {
            return Messages.WORLD_NOT_FOUND;
        }
     ...
     */
    @Nullable
    public static Message teleportToPlayerByUuid(
            @Nonnull Store<EntityStore> store,
        ...
        final TransformComponent targetTransform = targetStore.getComponent(targetRef, TransformComponent.getComponentType());

        if (targetTransform == null) {
            return Messages.CANNOT_GET_TARGET_PLAYER_POSITION;
        }

        final Vector3d targetPos = targetTransform.getPosition();

        final EntityStore targetEntityStore = targetStore.getExternalData();
        final World targetWorld = targetEntityStore.getWorld();

        final Vector3f rotation = cardinalRotation(targetTransform.getRotation());
