...
public class BaseTellCommand extends CommandBase {
    ...
    protected void executeSync(@NonNullDecl CommandContext commandContext) {
        commandContext.sendMessage(Message.join(
                ...
                Message.raw("/tell ").color(Color.WHITE).bold(true), Message.raw("<on/off>").color(Color.YELLOW).bold(true), Message.raw(StringUtils.padLeft("Alterna suas mensagens privadas\n", 34, " "))
        ));
    }
}
